/**
 * I don't think I'm using this right now. Should probably remove it?
...
async function getData(url) {
	...
	console.log(`data retreived in ${((endTime - startTime) / 1000).toFixed(2)} seconds`, data);
	return data;
};
...
export async function buildData() {
	...
	while (nextExists) {
		...
		console.log("allData so far", allData);
		...
		url = data.next;
		// uncomment the following line to only run the api call once
		// nextExists = false;
		console.log("Onto the next page...");
	}

	const endTime = performance.now();
	console.log(`allData compiled in ${((endTime - startTime) / 1000).toFixed(2)} seconds`, allData);
